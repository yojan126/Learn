...
namespace ExcelEditCore
{
    public class ExcelEdit
    {
        ...
        public Worksheet ws;
        int iWordsLen = 0;
        private List<System.Data.DataTable> LstdtExecl;
        NPOIHelper NHp = new NPOIHelper();
        private int iLstNum = 0;
        ...
        public Dictionary<string, string> GetExcelWords(ref bool bolContinue, Dictionary<string, string> dicAll)
        {
            iWordsLen = 0;
            Dictionary<string, string> dicWords = new Dictionary<string, string>();
            LstdtExecl = NPOIHelper.ImportExceltoListDt(mFilename);
            string str_Tmp = "";

            for (int l = iLstNum; l < LstdtExecl.Count(); l++)
            {
                for (int r = iRowNum; r < LstdtExecl[l].Rows.Count; r++)
                {
                    for (int c = iColNum; c < LstdtExecl[l].Columns.Count; c++)
                    {
                        str_Tmp = LstdtExecl[l].Rows[r][c].ToString();
                        if (!string.IsNullOrEmpty(str_Tmp))
                        {
                            string[] tmpWords = str_Tmp.Split(' ');
                            foreach (string tmpWord in tmpWords)
                            {
                                if (CheckWord(tmpWord) && !dicWords.ContainsKey(tmpWord) && !dicAll.ContainsKey(tmpWord) && tmpWord.Length > 1)
                                {
                                    dicWords.Add(tmpWord, "");
                                    iWordsLen = iWordsLen + tmpWord.Length + 3;
                                    if (iWordsLen >= 4000)
                                    {
                                        iLstNum = l;
                                        iRowNum = r;
        ...
        public void UpdateAndCreateExcel(Dictionary<string, string> dicWords,ref string strNewFile)
        {
            NPOIHelper.UpdateExcel(mFilename, LstdtExecl, dicWords,ref strNewFile);
        }
