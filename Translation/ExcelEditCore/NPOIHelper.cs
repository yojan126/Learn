...
{
    private static IWorkbook OpenedWorkBook;
    ...
    /// <returns></returns>
    public static List<DataTable> ImportExceltoListDt(string strFileName)
    {
        ...
        {
            OpenedWorkBook = wb = WorkbookFactory.Create(file);
        }
        int iSheetCount = wb.NumberOfSheets;
    ...
    /// <param name="lstWorkBook">需更新的数据</param>
    public static void UpdateExcel(string outputFile, List<DataTable> lstWorkBook, Dictionary<string, string> dicWords, ref string strNewFile)
    {
        //FileStream readfile = new FileStream(outputFile, FileMode.Open, FileAccess.Read);
        FileStream readfile = new FileStream(outputFile, FileMode.Open, FileAccess.Read);
        IWorkbook hssfworkbook;
        if (outputFile.IndexOf(".xlsx") > 0)
        {
        ...
        foreach (DataTable dt in lstWorkBook)
        {
            ...
            for (int r = 0; r < dt.Rows.Count; r++)
            {
                try
                {
                    if (Tmpsheet.GetRow(r) == null)
                    {
                        continue;
                    }
                    for (int c = 0; c < dt.Columns.Count; c++)
                    {
                        if (Tmpsheet.GetRow(r).GetCell(c) == null)
                        {
                            continue;
                        }
                        string strCell = Tmpsheet.GetRow(r).GetCell(c).ToString();
                        string[] strWords = strCell.Split(' ');
                        string strAdd = "";
                        foreach (string s in strWords)
                        {
                            if (dicWords.ContainsKey(s))
                            {
                                strAdd = strAdd + dicWords[s].ToString().Replace("“", "").Replace("”", "") + " ";
                            }
                            else
                            {
                                strAdd = strAdd + s + " ";
        ...
        try
        {
            ...
            int TmpIndex = outputFile.IndexOf(".xls");

            strNewFile = outputFile.Substring(0, TmpIndex) + "Trans" + outputFile.Substring(TmpIndex);//outputFile.Split('.')[0] + "Trans" + "." + outputFile.Split('.')[1];
            //保存为Excel文件  
            using (FileStream fs = new FileStream(strNewFile, FileMode.OpenOrCreate, FileAccess.Write))
            {
